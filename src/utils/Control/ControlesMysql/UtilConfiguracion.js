...
module.exports = {
    ...
    async deleteControlTotal(VALUES){
        try{          
    ...
    async createPlantillaTotal(VALUES, control_id, usuario_id){
        try{

            let nombre = await escapeDataSlashes(VALUES.nombre);
            let datos = await escapeDataSlashes(VALUES.datos);
            let precarga = await escapeDataSlashes(VALUES.precarga);
            let gc = await escapeDataSlashes(VALUES.gc);
            let acciones = await escapeDataSlashes(VALUES.acciones);
            let pesos = await escapeDataSlashes(VALUES.pesos);
            let pallets = await escapeDataSlashes(VALUES.pallets);
            let cajas = await escapeDataSlashes(VALUES.cajas);
            let documentacion = await escapeDataSlashes(VALUES.documentacion);
            let fotos = await escapeDataSlashes(VALUES.fotos);
            

            let cliente = await pool.query('SELECT control_cliente_id FROM control WHERE control_id = ?', [control_id]);
            let cliente_id = cliente[0].control_cliente_id;

            await pool.query('UPDATE control SET control_plantilla = ? WHERE control_id = ?', [1, control_id]);

            let insertedPlantilla = await pool.query('INSERT INTO plantilla (plantilla_usuario_id, plantilla_cliente_id, plantilla_control_id, plantilla_nombre, plantilla_tipo) VALUES (?,?,?,?,?)', [usuario_id, cliente_id, control_id, nombre, 1]);
            let hash = await genHash('plantilla', insertedPlantilla.insertId);
            await pool.query('UPDATE plantilla SET plantilla_id_hash = ? WHERE plantilla_id = ?', [hash, insertedPlantilla.insertId]);


            if(datos === '1'){
                await pool.query('UPDATE plantilla SET plantilla_datos = ? WHERE plantilla_usuario_id = ? AND plantilla_control_id = ? AND plantilla_nombre = ?', [1 ,usuario_id, control_id, nombre]);
            }

            if(precarga === '1'){
                await pool.query('UPDATE plantilla SET plantilla_precarga = ? WHERE plantilla_usuario_id = ? AND plantilla_control_id = ? AND plantilla_nombre = ? ', [1 ,usuario_id, control_id, nombre]);
            }

            if(gc === '1'){
                await pool.query('UPDATE plantilla SET plantilla_gc = ? WHERE plantilla_usuario_id = ? AND plantilla_control_id = ? AND plantilla_nombre = ?', [1 ,usuario_id, control_id, nombre]);
            }

            if(acciones === '1'){
                await pool.query('UPDATE plantilla SET plantilla_acciones = ? WHERE plantilla_usuario_id = ? AND plantilla_control_id = ? AND plantilla_nombre = ?', [1 ,usuario_id, control_id, nombre]);
            }

            if(pesos === '1'){
                await pool.query('UPDATE plantilla SET plantilla_pesos = ? WHERE plantilla_usuario_id = ? AND plantilla_control_id = ? AND plantilla_nombre = ?', [1 ,usuario_id, control_id, nombre]);
            }

            if(pallets === '1'){
                await pool.query('UPDATE plantilla SET plantilla_pallets = ? WHERE plantilla_usuario_id = ? AND plantilla_control_id = ? AND plantilla_nombre = ?', [1 ,usuario_id, control_id, nombre]);
            }

            if(cajas === '1'){
                await pool.query('UPDATE plantilla SET plantilla_cajas = ? WHERE plantilla_usuario_id = ? AND plantilla_control_id = ? AND plantilla_nombre = ?', [1 ,usuario_id, control_id, nombre]);
            }

            if(fotos === '1'){
                await pool.query('UPDATE plantilla SET plantilla_fotos = ? WHERE plantilla_usuario_id = ? AND plantilla_control_id = ? AND plantilla_nombre = ?', [1 ,usuario_id, control_id, nombre]);
            }

            if(documentacion === '1'){
                await pool.query('UPDATE plantilla SET plantilla_documentacion = ? WHERE plantilla_usuario_id = ? AND plantilla_control_id = ? AND plantilla_nombre = ?', [1 ,usuario_id, control_id, nombre]);
            }


            return {status: "ok"};
        }catch(error){
            return {status:'error', code: 'Error al crear la plantilla'};
        }
    },

    async deletePlantillaTotal(plantilla_id_hash, control_id){
        try{
          
            await pool.query('DELETE FROM plantilla WHERE plantilla_id_hash = ? AND plantilla_control_id = ?',[plantilla_id_hash, control_id]);

            return {status: "ok"};
        }catch(error){
            return {status:'error', code: 'Error al eliminar la plantilla'};
        }
    }
