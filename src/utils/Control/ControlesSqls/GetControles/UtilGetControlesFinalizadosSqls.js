...
module.exports = {
    ...
    async getControlesFinalizadosListSqls(PARAMS, inicio_pagina) {
        ...
        try {
            ...
            if (rol_id === 1) {//admin
                ...
                controles = await mssql.query(`SELECT Control_idGuid AS control_id, 
                                            ...
                                            Familia_idGuid,
                                            Marca AS control_producto_marca
                                            FROM control WHERE Usuario_idGuid = '`+ usuario_idSQLS + `' AND finalizado = 1 ORDER BY Fecha_Control DESC OFFSET ` + inicio_pagina + ` ROWS FETCH NEXT 15 ROWS ONLY`);
                controles = controles.recordset;
