...
function generateControlPlantilla(plantillaId){
    ...
    $.ajax({
        url:   '/intranet/plantillas/en/'+plantillaId+'/nuevo/control',
        headers: {
            'CSRF-Token': token
        },
        type:  'POST',
        ...
        success:  function (response) {
            ...
            if(response.status === 'ok'){
                window.location.href = "https://www.pruebas.justq.eu/intranet/control/en"+response.hash;
            }else{
...
function paginacion() {
	...
	const link = '/intranet/plantillas/en';
