...
function uploadDatos(controlId, value, type){
	...
	var token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
		...
		$.ajax({
			data:  parametros,
			url:   '/intranet/control/en/'+controlId+'/informacion/mod',
			headers: {
				'CSRF-Token': token
			},
			type:  'POST',
...
function deleteControl(controlId, tecnicoId){
	window.alert('If the control is deleted, the data cannot be recovered')
	if(window.confirm("Delete control?")) {
		...
		$.ajax({
			data:  parametros,
			url:   '/intranet/control/en/'+controlId+'/eliminar', 
			headers: {
				'CSRF-Token': token
			},
			type:  'POST',
			...
			success:  function (response) {
				...
				if(response.status === 'ok'){
					window.location.href = '/intranet/controles/en';
				}else{
...
function deletePlantilla(controlId, plantillaId){
	if(window.confirm("Delete template?")) {
		...
		$.ajax({
			url:   '/intranet/control/en/'+controlId+'/delete/plantilla/'+plantillaId, 
			headers: {
				'CSRF-Token': token
			},
			type:  'delete',
