...
module.exports = {
    ...
    async getFotosEn(req, res){
        ...
        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            ...
            switch(data.status){
                case 'ok':
                    res.render(modules+'en/fotos', {layout: layout+'en/fotos.hbs', csrfToken: req.csrfToken(), menuControl, activePrecarga, menuFotos, data, control_codigo, control_id: cnf.control_hash, user, nav});
                    break;
                case 'error':
    ...
    async postFotosEn(req, res){
        ...
        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            ...
            if(response.status === 'ok'){
                return res.redirect('/intranet/control/en/'+cnf.control_hash+'/fotos');
            }else{
                return res.redirect('/intranet/control/en/'+cnf.control_hash+'/fotos');
            }
        }else{
    ...
    async deleteAllFotosEn(req, res){
        const cnf = await requested(req);
