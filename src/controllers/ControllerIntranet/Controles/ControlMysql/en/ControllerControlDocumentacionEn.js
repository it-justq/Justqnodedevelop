...
module.exports = {
    ...
    async getDocumentacionEn(req, res){
        ...
        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            ...
            switch(data.status){
                case 'ok':
                    res.render(modules+'en/documentacion', {layout: layout+'en/documentacion.hbs', csrfToken: req.csrfToken(), menuControl, activePrecarga, menuDocumentacion, data, control_codigo, control_id: cnf.control_hash, user, nav});
                    break;
                case 'error':
    ...
    async postDocumentacionEn(req, res){
        const cnf = await requested(req);
        ...
        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            ...
            if(response.status === 'ok'){
                return res.redirect('/intranet/control/en/'+cnf.control_hash+'/documentacion');
            }else{
                return res.redirect('/intranet/control/en/'+cnf.control_hash+'/documentacion');
            }
        }else{
    ...
    async deleteAllDocumentacionEn(req, res){
        const cnf = await requested(req);
    ...
    async deleteDocumentacionEn(req, res){
        const cnf = await requested(req);
