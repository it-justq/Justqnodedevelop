...
module.exports = {
    ...
    async getDatos(req, res){
        ...
        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            ...
            switch(data.status){
                case 'ok':
                    res.render(modules+'datos', {layout: layout+'datos.hbs', csrfToken: req.csrfToken(), menuControl, activePrecarga, menuDatos, data, control_codigo, control_id: cnf.control_hash, user, nav});
                break;
                case 'error':
                    res.redirect('/ERROR/control/'+cnf.control_hash+'/datos');
                break;
            }
