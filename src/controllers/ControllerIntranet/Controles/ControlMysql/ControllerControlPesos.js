...
module.exports = {
    ...
    async getPesos(req, res){
        ...
        if(await await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            ...
            switch(data.status){
                case 'ok':
                    res.render(modules+'pesos', {layout: layout+'pesos.hbs', csrfToken: req.csrfToken(), menuControl, activePrecarga, menuPesos, data, control_codigo, control_id: cnf.control_hash, user, nav});
                    break;
                case 'error':
    ...
    async getPesoFotos(req, res){
        ...
        if(await permisosControl(cnf.control_id, user.id, user.rol)){
            ...
            switch(data.status){
                case 'ok':
                    res.render(modules+'peso-fotos', {layout: layout+'peso-fotos.hbs', csrfToken: req.csrfToken(), menuControl, activePrecarga, menuPesoFotos, data, peso_id: cnf.id, control_codigo, control_id: cnf.control_hash, user, nav});
                    break;
                case 'error':
                    res.redirect('/ERROR/control/'+cnf.control_hash+'/pesos');
                break;
    ...
    async postPesoFotos(req, res){
        const cnf = await requested(req);

        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            let response = await postDatosPesoFotos(cnf);
            if(response.status === 'ok'){
                return res.redirect('/intranet/control/'+cnf.control_hash+'/peso/'+cnf.id+'/fotos');
            }else{
                return res.redirect('/intranet/control/'+cnf.control_hash+'/peso/'+cnf.id+'/fotos');
            }
        }else{
