...
module.exports = {
    ...
    async getPallets(req, res){
        ...
        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            ...
            switch(data.status){
                case 'ok':
                    res.render(modules+'pallets', {layout: layout+'pallets.hbs', csrfToken: req.csrfToken(), menuControl, activePrecarga, menuPallets, data, control_codigo, control_id: cnf.control_hash, user, nav});
                    break;
                case 'error':
    ...
    async getPalletFotos(req, res){
        ...
        if(await permisosControl(cnf.control_id, user.id, user.rol)){
            ...
            switch(data.status){
                case 'ok':
                    res.render(modules+'pallet-fotos', {layout: layout+'pallet-fotos.hbs', csrfToken: req.csrfToken(), menuControl, activePrecarga, menuPalletFotos, data, pallet_id: cnf.id, control_codigo, control_id: cnf.control_hash, user, nav});
                    break;
                case 'error':
                    res.redirect('/ERROR/control/'+cnf.control_hash+'/pallets');
                break;
    ...
    async postPalletFotos(req, res){
        const cnf = await requested(req);

        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            let response = await postDatosPalletFotos(cnf);
            if(response.status === 'ok'){
                return res.redirect('/intranet/control/'+cnf.control_hash+'/pallet/'+cnf.id+'/fotos');
            }else{
                return res.redirect('/intranet/control/'+cnf.control_hash+'/pallet/'+cnf.id+'/fotos');
            }
        }else{
    ...
    async deleteFotosPallet(req, res){
        const cnf = await requested(req);
    ...
    async deleteFotoPallet(req, res){
        const cnf = await requested(req);
        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
