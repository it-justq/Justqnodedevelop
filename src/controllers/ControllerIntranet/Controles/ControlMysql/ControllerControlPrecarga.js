...
module.exports = {
    ...
    async getPrecarga(req, res){
        ...
        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol) && activePrecarga){
            ...
            switch(data.status){
                case 'ok':
                    res.render(modules+'precarga', {layout: layout+'precarga.hbs', csrfToken: req.csrfToken(), menuControl, activePrecarga, menuPrecarga, data, control_codigo, control_id: cnf.control_hash, user, nav});
                    break;
                case 'error':
                    res.redirect('/ERROR/control/'+cnf.control_hash+'/precarga');
                break;
            }
