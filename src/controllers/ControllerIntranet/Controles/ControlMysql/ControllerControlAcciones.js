...
module.exports = {
    ...
    async getAcciones(req, res){
        ...
        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            ...
            switch(data.status){
                case 'ok':
                    res.render(modules+'acciones', {layout: layout+'acciones.hbs', csrfToken: req.csrfToken(), menuControl, menuAcciones, activePrecarga, data, control_codigo, control_id: cnf.control_hash, user, nav});
                    break;
                case 'error':
    ...
    async modAcciones(req, res){
        const cnf = await requested(req);

        if(await permisosControl(cnf.control_id, cnf.user_id, cnf.rol)){
            await modAccionesDatos(cnf);
            res.redirect('/intranet/control/'+cnf.control_hash+'/acciones');
        }else{
