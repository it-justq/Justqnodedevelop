...
module.exports = {
    ...
    async getPaisesSearch(req, res){
        ...
        if(await isPermissedTo(cnf.user_id, cnf.rol, 'ADMIN-PAISES')){
            ...
            if(data.status = 'error'){
                data = data.data;
                res.render(modules+'main', {layout: layout+'main.hbs', csrfToken: req.csrfToken(), data, menuAdmin, paises_search: true, user, nav});
            }else{
                res.redirect('/ERROR/unauthorized');
            }
