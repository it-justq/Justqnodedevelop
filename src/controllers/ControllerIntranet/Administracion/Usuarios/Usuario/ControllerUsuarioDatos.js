...
module.exports = {
    ...
    async getDatosUsuario(req, res){
        
        const cnf = requested(req);

        if(await isPermissedTo(cnf.user_id, cnf.rol, 'ADMIN-USUARIOS')){

            const data = await getUsuarioData(cnf.id, 'es');
            const user = getUserData(req);

            res.render(modules+'datos', {layout: layout+'datos.hbs', csrfToken: req.csrfToken(), data, menuAdmin, pagina: cnf.pagina, user, nav});
        }else{
    ...
    async postDatosUsuario(req, res){
