...
module.exports = {
    ...
    async getPlantillasEn(req, res){
        ...
        switch(data.status){
            case 'ok':
                res.render(modules+'plantillas/en/main', {layout: layout+'plantillas/en/main.hbs', csrfToken: req.csrfToken(), data, pagina, user, nav});
                break;
            case 'error':
                res.redirect('');
            break;
    ...
    async postNuevoControlPlantillaEn(req, res){
        const user = getUserData(req);
